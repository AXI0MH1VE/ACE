...
- Expected: Fused SSM+attention sample with `mode="creative"` and `request_id`.
...
- Request: `POST /api/v1/verified` with `{ "prompt": "...", "axiom_set": "finance.gaap2025", "max_steps": 4096, "payment_token": "<ln-invoice>", "allow_network": true, "free_local": false }`
- Expected: Deterministic output with proof URI, C0 signature, and Merkle root; request denied if payment is missing or policy blocks outbound access.
